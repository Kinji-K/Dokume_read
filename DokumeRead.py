...
def database_create(con):
    try:
        # テーブルの作成
        con.execute("CREATE TABLE IF NOT EXISTS bookinfo (id TEXT PRIMARY KEY, title TEXT, auther TEXT, read_number INTERGER DEFAULT 0)")
        con.execute("CREATE TABLE IF NOT EXISTS read_data (id TEXT, date INTEGER, note TEXT)")
        con.execute("CREATE TABLE IF NOT EXISTS bookcase (id TEXT PRIMARY KEY, case1 TEXT DEFAULT '', case2 TEXT DEFAULT '', case3 TEXT DEFAULT '', case4 TEXT DEFAULT '', case5 TEXT DEFAULT '', case6 TEXT DEFAULT '', case7 TEXT DEFAULT '', case8 TEXT DEFAULT '', case9 TEXT DEFAULT '', case10 TEXT DEFAULT '')")
    except sqlite3.Error as e:
        print('Database error')

# 読書メーターログイン用関数
def dokume_login(s, login_data):
...
def get_read_bookids(s,bookids,userid,BOOKS_IN_PAGE):
    ...
    while n > BOOKS_IN_PAGE * i:
        # 読んだ本リストのページへのリンク作成
        url = 'https://bookmeter.com/users/' + userid + '/books/read?page={}'.format(i+1)
...
def DokumeRead():
    ...
    dokume_login(session,login_data)

    # 読んだ本のidと読んだ回数を取得
    get_read_bookids(session,bookids,userinfo[0],BOOKS_IN_PAGE)
